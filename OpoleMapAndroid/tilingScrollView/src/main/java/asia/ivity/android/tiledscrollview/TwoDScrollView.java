...
public class TwoDScrollView extends FrameLayout
{
    ...
    private static final String TAG = TwoDScrollView.class.getSimpleName();

    private boolean mLastScrollStatus;

    /**
     * Immediate status after scroll has been called
     * @return
     */
    public boolean getLastScrollStatus()
    {
        return mLastScrollStatus;
    }
    ...
    {
        mLastScrollStatus = false;
        // we rely on the fact the View.scrollBy calls scrollTo.
        if (getChildCount() > 0)
        {

            View child = getChildAt(0);
            final int myWidth = getWidth() - getPaddingRight() - getPaddingLeft();
            final int myHeight = getHeight() - getPaddingBottom() - getPaddingTop();

            final int childWidth = child.getWidth();
            final int childHeight = child.getHeight();

            x = clamp(x, myWidth, childWidth);
            y = clamp(y, myHeight, childHeight);

            if ((x != getScrollX()) || (y != getScrollY()))
            {
                // #ifdef DEBUG
                Log.v(TAG, "scrollTo(x: " + x + ", y: " + y + ')');
                // #endif
                mLastScrollStatus = true;
                super.scrollTo(x, y);
            }
        }
    }
